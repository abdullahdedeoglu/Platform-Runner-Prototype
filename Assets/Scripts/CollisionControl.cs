...
public class CollisionControl : MonoBehaviour
{
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        {
            // Platformdaki hareketi zorlaştırmak için kuvvet uygula
    ...
    private void OnCollisionExit(Collision collision)
    {
        if (collision.gameObject.CompareTag("RotatingPlatform"))
        {
            // Platformdan ayrıldığında hızları sıfırla
            ResetPlayerVelocity();
        }
    }
    ...
    private IEnumerator HandleDeathAndRespawn()
    {
        ...
        // Karakteri yeniden başlatma konumuna taşı
        if (isPlayer)
            characterMovement.SetIsAlive();
        else if(!isPlayer)
        {       
